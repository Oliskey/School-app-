-- MANUAL FIX V2 (Corrected ID Lookup)
-- Run this entire script in Supabase SQL Editor to fix Chat and CBT Upload.

BEGIN;

-- ==========================================
-- PART 1: CHAT SCHEMA (Using simple policies)
-- ==========================================
...
-- Permissive Chat Policies (Using 'true' to avoid type errors and ensure access)
...
-- ==========================================
-- PART 2: CBT EXAMS RLS FIX (Using Email Lookup)
-- ==========================================

-- Fix cbt_exams Policies
ALTER TABLE IF EXISTS public.cbt_exams ENABLE ROW LEVEL SECURITY;

-- INSERT: Allow any authenticated user (Teachers) to create exams
...
-- SELECT: Allow any authenticated user to view exams
...
-- UPDATE: Allow only the creator (Teacher) to update
-- CORRECTION: Use Email lookup since teachers table uses ID (BigInt) but Auth is UUID
DROP POLICY IF EXISTS "Enable update for creators" ON public.cbt_exams;
CREATE POLICY "Enable update for creators" ON public.cbt_exams FOR UPDATE USING (
    teacher_id IN (
...
-- DELETE: Allow only the creator to delete
DROP POLICY IF EXISTS "Enable delete for creators" ON public.cbt_exams;
CREATE POLICY "Enable delete for creators" ON public.cbt_exams FOR DELETE USING (
     teacher_id IN (
        SELECT id FROM public.teachers 
        WHERE email = (SELECT email FROM auth.users WHERE id = auth.uid())
    )
